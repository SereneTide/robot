"""
机械臂控制模块 —— KUKA IIWA的初始化、运动控制、逆运动学。
"""
...
class SimRobot:
    """KUKA IIWA机械臂控制：加载、姿态控制、末端移动、抓取。"""
    ...
    def _get_joint_positions(self) -> list:
        states = p.getJointStates(self.robot_id, JOINT_INDICES,
                                   physicsClientId=self.client_id)
        return [s[0] for s in states]

    def _joints_reached(self, target: list) -> bool:
        current = self._get_joint_positions()
        return all(abs(c - t) <= JOINT_TOLERANCE for c, t in zip(current, target))
    ...
    def move_one_point(self, x: float, y: float, z: float,
        ...
        """
        末端平滑移动至目标空间坐标（线性插值→IK→逐点控制）。
        返回: (success: bool, trajectory: list)
        """
        if steps is None:
            steps = MOVE_STEPS
